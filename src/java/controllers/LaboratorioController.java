...
import facades.LaboratorioFacade;
import util.JsfUtil;
...
public class LaboratorioController implements Serializable {
    ...
    @EJB
    private facades.LaboratorioFacade ejbFacade;
    ...
    public SelectItem[] getItemsAvailableSelectOne() {
        ...
        laboratorioList = getFacade().findAll();

        return LaboratorioController.getSelectItems(laboratorioList);
    }
    ...
    public static SelectItem[] getSelectItems(List<Laboratorio> entities) {
        int size = entities.size();
        SelectItem[] items = new SelectItem[size];
        int i = 0;

        for (Laboratorio x : entities) {
            items[i++] = new SelectItem(x, x.getNome());
        }
        return items;
    }
}
