...
public class PedidosController implements Serializable {
    ...
    public void gerarPedido() {
        ...
        somaPedidos = getFacade().reportQuery(listChecked);
    ...
    public Pedidos prepareCreate() {
        ...
        Date today = new Date();
        this.selected.setData(today);
        this.selected.setAberto(true);
        return selected;
    }
    ...
    public List<Pedidos> getItems() {
        if (items == null) {
            items = getFacade().findByAberto(Boolean.TRUE);
        }
        return items;
    }
        ...
        if (selected != null) {
            ...
            try {
                if (persistAction == PersistAction.CREATE) {
                    ...
                    PedidosDetailController pedidosDetailController = (PedidosDetailController) elContext.getELResolver().getValue(elContext, null, "pedidosDetailController");
                    pedidosDetailController.selecionaPedidos(selected.getIdpedidos());
    ...
    public List<Pedidos> getItemsAvailableSelectMany() {
        return getFacade().findByAberto(Boolean.TRUE);
    }

    public SelectItem[] getItemsAvailableSelectOne() {

        List<Pedidos> pedidosList = getFacade().findByAberto(Boolean.TRUE);
        Collections.sort(pedidosList, Comparators.PEDIDOSID);

        return PedidosController.getSelectItems(pedidosList);
    }
    ...
    public Pedidos getSomaPedidos() {
        return somaPedidos;
    }

    public void setSomaPedidos(Pedidos somaPedidos) {
